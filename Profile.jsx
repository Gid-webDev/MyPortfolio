...
const Profile = ({ selectedMode, textMode, BGcolor}) => {

    const {setProfile, showProfile} = useContext(ProfileContext);
    const {navBarOn, setNavBarOn} = useContext(DataContext)
    ...
    const myIntro = showProfile ===true? {
      ...
      boxShadow: `0px 0px 5px ${textMode}` } : 
    ...
    {name: 'MongoDB', id: 12, Icon: 'mongo.png'},
]
  ...
  return (
    <>
        ...
        <div className='d-flex align-items-center rounded-5 py-2'>
             ...
             <button className='btn fs-4 ' onClick={()=> showProfile? setProfile(false, setNavBarOn(true)) : setProfile(true, setNavBarOn(false))}
             style={{position:'absolute', top:'10px', right:'-20px', color: textMode}}>
                <AiOutlineClose/>
               </button>
            </div>
